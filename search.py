...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        # stepCount plus turn cost
        elif (self.env.costFunc == costFunctions[1]):
            # print("path_cost: For students to implement")

            #print(curNode.action)
            cost += self.computeTurnCost(curNode.action,action) + 1

        # cost function to encourage staying left of grid
        elif (self.env.costFunc == costFunctions[2]):
            # print("path_cost: For students to implement")
            if state2[0] <= (self.env.width/2):
                cost += self.computeTurnCost(curNode.action,action)
            else:
                left_cleaned = True
                for dirty_cell in self.env.dirtyRooms:
                    # print("Checking .dirtyRooms")
                    if dirty_cell[0] <= (self.env.width/2):
                        # print("a cells in list is on left")
                        left_cleaned = False
                if left_cleaned:
                    # print("left half all cleaned")
                    cost += self.computeTurnCost(curNode.action, action)
                else:
                    cost += self.computeTurnCost(curNode.action,action) + 5

        # cost function to encourage staying top of grid
        else:  #  == costFunctions[3].
            # print("path_cost: For students to implement")
            if state2[1] >= (self.env.height / 2):
                ...
                for dirty_cell in self.env.dirtyRooms:
                    #print("Checking .dirtyRooms")
                    if dirty_cell[1] >= (self.env.width / 2):
                        #print("a cells in list is on top")
                        top_cleaned = False
                if top_cleaned:
                    #print("~~ top half all cleaned ~~")
                    cost += self.computeTurnCost(curNode.action, action)
                else:
                    cost += self.computeTurnCost(curNode.action, action) + 5
    ...
    def computeTurnCost(self, action1, action):
        ...
        # To ret the 180 deg of input's direction
        ...
        if action1 == action:
            return 0
        # turning 180 deg, cost 6
        elif reverted_direction(action1) == action:
            return 6
        # turning 90 deg, cost 3
        return 3
    ...
    def findMinManhattanDist(self, pos):
        ...
        # note: room = a cell, pos = current state
        min_dist = math.inf

        # loop through entire map's grey cell
        for cell in self.env.dirtyRooms:
            # find grey cell that closest to current posit
            min_dist = min(distance_manhattan(pos, cell), min_dist)
    ...
    def findMinEuclidDist(self, pos):
        ...
        for cell in self.env.dirtyRooms:
            # find grey cell that closest to current posit
            min_dist = min(distance_squared(pos, cell), min_dist)
...
def breadth_first_graph_search(problem):
    ...
    # Initialize the node, and check if is goal
    node = Node(problem.initial)
    if problem.goal_test(node.state):
        return node, None

    # Queue (Fin,Fout), put initial node in frontier
    frontier = deque([node])
    explored = set()

    # while frontier is not empty
    while frontier:
        # popleft = pop from left(First in)
        ...
        for child in node.expand(problem):
            ...
            # if haven't explore this child AND not in frontier
            if tuple(s) not in explored and child not in frontier:
                # check if this child is goal state
...
def depth_first_graph_search(problem):
    ...
    # Initialize the node, check if is goal
    node = Node(problem.initial)
    if problem.goal_test(node.state):
        return node, None

    # Stack (Lin,Fout), put initial node in frontier
    frontier = deque([node])
    explored = set()

    # while frontier is not empty
    while frontier:
        # deque's pop = pop from right(Last in)
        node = frontier.pop()
        explored.add(tuple(node.state))
        ...
        for child in node.expand(problem):
            ...
            # if haven't explore this child AND not in frontier
            if child not in frontier and tuple(s) not in explored:
                # check if this child is goal state
                if problem.goal_test(s):
                    return child, explored
                else:
...
def best_first_graph_search(problem, f=None):
    ...
    while frontier:
        ...
        for child in node.expand(problem):
            ...
            if child not in frontier and tuple(s) not in explored:
                frontier.append(child)
            # child is in frontier, but new < in_frontier cost
